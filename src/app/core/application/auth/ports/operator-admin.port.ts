 ...
 *
 * Read-only for now (story #44, phase 1). Mutating operations — invite,
 * deactivate, role reassignment — are deferred so they can be designed
 * alongside the in-flight multi-tenant membership work (#43) rather than
 * racing it.
 */
export interface OperatorAdminPort {
   ...
   * missing operator or a role name the domain does not recognise is skipped
   * (logged, not thrown), so one bad row cannot break the whole screen.
   */
