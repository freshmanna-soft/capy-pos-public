...
import { DexieDatabase, ITransactionDB } from '../database/dexie-database.service';
...
export class DexieTransactionRepository 
  ...
  protected mapToEntity(record: ITransactionDB): Transaction {
    ...
    return new Transaction(
      ...
      record.cancelledAt ? new Date(record.cancelledAt) : undefined,
   ...
   */
  async getSalesByHour(date: Date): Promise<Array<{
    hour: number;
    sales: number;
    transactions: number;
  }>> {
   ...
   */
  async getRefundStats(startDate: Date, endDate: Date): Promise<{
    totalRefunds: number;
    refundCount: number;
    refundRate: number;
  }> {
    ...
    const completedSales = records.filter(r => 
      r.status === TransactionStatus.COMPLETED && r.type === TransactionType.SALE
    );
    
    const refundedTransactions = records.filter(r =>
  ...
  async findByProduct(
    ...
    let query = this.table.filter(record => {
