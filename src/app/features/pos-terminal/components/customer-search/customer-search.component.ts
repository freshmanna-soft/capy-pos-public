...
@Component({
  ...
  template: `
    <div class="customer-search-container">
      ...
      <!-- Loading -->
      @if (isLoading()) {
        <div data-testid="search-loading" class="loading">Searching customers...</div>
      }

      <!-- Error -->
      @if (error()) {
        <div data-testid="search-error" class="error" role="alert">
          {{ error() }}
        </div>
      }
      ...
      @if (searchResults().length > 0 && !isLoading()) {
        <div class="search-results" role="listbox">
          @for (customer of searchResults(); track customer; let i = $index) {
            <div
              [id]="'result-' + i"
              ...
              (keydown.enter)="selectItem(customer)"
      ...
      @if (searchQuery().length >= 2 && searchResults().length === 0 && !isLoading() && !error()) {
        <div data-testid="no-results" class="no-results">No customers found</div>
      }
    </div>
  `,
  styles: [
...
export class CustomerSearchComponent extends BaseSearchComponent<Customer> {
  ...
  protected override formatError(_error: unknown): string {
  ...
  protected override afterItemSelect(customer: Customer): void {
    console.log(
      '[CustomerSearch] Selected customer:',
      customer.name,
      'with',
